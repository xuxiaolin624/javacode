...
public interface UserDao {
	...
	// 多条件查询
	@Select("<script>" + "select * from user " + "<where> " + "<if test='keyWord != \"\" and keyWord != null'>"
			+ " and (user_id like '%${keyWord}%' or user_name like '%${keyWord}%'  ) "// 单匹配
			+ "</if>" + "</where>" + "<choose>" + "<when test='orderBy != \"\" and orderBy != null'>"
			+ " order by ${orderBy} ${sort}" + "</when>" + "<otherwise>" + " order by userId desc" + "</otherwise>"
			+ "</choose>" + "</script>")
